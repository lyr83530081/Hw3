# -*- coding: utf-8 -*-
"""
Created on Thu Apr 15 06:07:27 2021
...
for i in range(max_epochs):
    ...
    loss_t = 0
    while(True):
        batch,batch_lable = data_loader(index,batch_size,k,Y_list)
        batch = np.array(batch).transpose((0,3,1,2))/255
        grads = network.gradient(batch, batch_lable)
        optimizer.update(network.params, grads)
        loss = network.loss(batch, batch_lable)
        loss_t+=loss
        print("batch:"+str(k)+" train loss:" + str(loss))  
        k+=1
        if k==len(index)//batch_size+2:
            break
    loss_train.append(loss_t/(k-1)) 
    correct_val = 0
    k=1
    loss_v = 0
    while(True):
